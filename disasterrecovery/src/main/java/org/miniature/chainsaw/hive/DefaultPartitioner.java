...
import org.miniature.chainsaw.hive.util.EventSourcerUtils;
import org.miniature.chainsaw.hive.util.HiveDRUtils;
import org.miniature.chainsaw.hive.util.ReplicationStatus;
...
public class DefaultPartitioner implements Partitioner {
    ...
    public ReplicationEventMetadata partition(final HiveDROptions drOptions, final String databaseName,
        ...
        while (taskIter.hasNext()) {
            ...
            if (task.isActionable()) {
                ...
                Iterable<? extends Command> srcCmds = task.getSrcWhCommands();
                if (srcCmds != null) {
                ...
                Iterable<? extends Command> dstCmds = task.getDstWhCommands();
                if (dstCmds != null) {
                    if (eventScope == Scope.DB) {
    ...
    private void processDBScopeCommands(final List<Command> dbEventList, final Iterable<? extends Command> cmds, final Map<String, List<OutputStream>> outputStreamMap, CMDTYPE cmdType
    ) throws Exception {
    ...
    private void processTableScopeCommands(final Iterable<? extends Command> cmds,
                                           final Map<String,
    ...
    private void addCmdsToDBEventList(List<Command> dbEventList, final Iterable
            <? extends Command> cmds) {
    ...
    private void addDbEventToAllTablesEventFile(
            final Iterable<? extends Command> cmds,
