...
public class HiveDRStatusStore extends DRStatusStore {
    ...
    public ReplicationStatus getReplicationStatus(String source, String target,
                                                  ...
                                                  String table) throws DRException {
    ...
    @Override
    public void deleteReplicationStatus(String jobName, String database) throws DRException {
        ...
        try {
            ...
            throw new DRException("Failed to delete status for Job "
    ...
    private DBReplicationStatus getDbReplicationStatus(String source, String target, String jobName,
                                                       String database) throws DRException {
    ...
    private void writeStatusFile(DBReplicationStatus dbReplicationStatus) throws DRException {
    ...
    public void rotateStatusFiles(Path statusDir, int numFiles, int maxFileAge) throws DRException {
    ...
    private DBReplicationStatus readStatusFile(Path statusDirPath) throws DRException {
        try {
    ...
    public void checkForReplicationConflict(String newSource, String jobName,
                                            String database, String table) throws DRException {
        try {
            ...
            for (FileStatus file : files) {
                ...
                if (!(newSource.equals(existingJob.getSourceUri()))) {
                    throw new DRException("Two different sources are attempting to replicate to same db "
                ...
                if (StringUtils.isEmpty(table)) {
                    ...
                    throw new DRException("Two different jobs are attempting to replicate to same db "
                ...
                for (Map.Entry<String, ReplicationStatus> entry : dbFileStatus.getTableStatuses().entrySet()) {
                    if (table.equals(entry.getKey())) {
                        throw new DRException("Two different jobs are trying to replicate to same table "
            ...
            throw new DRException("Failed to read status files for DB "
                    + database, e);
        }
    }
}
