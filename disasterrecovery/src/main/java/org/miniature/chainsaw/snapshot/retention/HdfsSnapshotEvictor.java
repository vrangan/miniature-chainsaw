...
package org.miniature.chainsaw.snapshot.retention;
...
import org.miniature.chainsaw.snapshot.retention.util.ExpressionHelper;
import org.miniature.chainsaw.snapshot.util.HdfsSnapshotMirrorProperties;
import org.miniature.chainsaw.snapshot.util.HdfsSnapshotUtil;
import org.miniature.chainsaw.util.OozieActionConfigurationHelper;
...
public class HdfsSnapshotEvictor extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
        ...
        DistributedFileSystem sourceFs = HdfsSnapshotUtil.getSourceFileSystem(sourceStorageUrl,
        ...
        if (retPolicy.equalsIgnoreCase("delete")) {
            ...
            throw new DRException("Unsupported source retention policy " + retPolicy);
        }
        ...
        if (retPolicy.equalsIgnoreCase("delete")) {
            ...
            throw new DRException("Unsupported target retention policy " + retPolicy);
        }
    ...
    protected static void evictSnapshots(DistributedFileSystem fs, String dirName, String ageLimit,
                                         int numSnapshots) throws DRException {
        try {
            LOG.info("Started evicting snapshots on dir {}{} using policy {}, agelimit {}, numSnapshot {}",
                    fs.getUri(), dirName, ageLimit, numSnapshots);

            long evictionTime = System.currentTimeMillis() - evalExpressionToMilliSeconds(ageLimit);
            ...
            throw new DRException("Unable to parse retention age limit " + ageLimit, ele);
        } catch (IOException ioe) {
            LOG.warn("Unable to evict snapshots from dir {} {}", dirName, ioe);
            throw new DRException("Unable to evict snapshots from dir " + dirName, ioe);
        }

    }

    private static Long evalExpressionToMilliSeconds(String period) throws ELException {
        ExpressionEvaluator EVALUATOR = new ExpressionEvaluatorImpl();
        ExpressionHelper RESOLVER = ExpressionHelper.get();
